...
Dashboard 增强 (4功能)       ████████████████████ 100%  (PnL图表 + 策略切换 + 参数热调 + 市场评分)
...
测试                        ████████████████████  95%  (236 Dashboard tests, 90 brokerage tests)
```
...
### Dashboard 增强 — 4 功能 (计划外增量)

| # | 功能 | 状态 | 说明 |
|---|------|------|------|
| E.1 | PnL 曲线图 | **DONE** | Chart.js v4 实时 equity curve + 回测多策略曲线对比 (Live/Backtest 切换) |
| E.2 | 多策略切换 UI | **DONE** | Header 下拉框运行时切换策略 + 可选状态重置，工厂模式创建策略实例 |
| E.3 | 策略参数热调整 | **DONE** | `GetParameters()` 接口方法 (4策略实现) + 动态表单 UI + `PUT /api/strategy/parameters` 热更新 |
| E.4 | 市场评分可视化 | **DONE** | `GetMarketScores()` 接口方法 + MarketScore model + 水平 score bar 排名列表，选中/持仓标记 |
| E.5 | 新 API 端点 (7个) | **DONE** | equity-curve, backtest/equity-curves, strategy GET/PUT, strategy/parameters GET/PUT, strategy/market-scores |
| E.6 | 增强测试 | **DONE** | 20 个新测试 (GetParameters roundtrip ×4, expected keys ×4, MarketScores ×6, models ×4, AvailableStrategies ×1, question lookup ×1) |
...
| T.10 | Dashboard 测试 | **DONE** | 236 个测试 — MarketMakingStrategy (20)、MeanReversionStrategy (17)、SpreadCaptureStrategy (15)、DryRunModels (14)、ApiClientRetryTests (8)、OrderBookStalenessTests (3)、BtcPriceServiceTests (16)、BtcFollowMMStrategyTests (42, 含 TTE 7 + Asymmetry 5 + Sentiment 7)、CorrelationMonitorTests (10)、SentimentServiceTests (44, 含分类 19 + 信号 7 + 状态 4 + 乘数 6 + 偏移 5 + HTTP 4)、BacktestTests (27)、DashboardEnhancementTests (20, GetParameters ×8 + MarketScores ×8 + Models ×4) |
...
**7 天回测结果** (2026-02-24 ~ 2026-03-03, 60 tokens, 38,161 bars, 983 ticks, 2.4s):
